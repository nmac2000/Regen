...
clim2016.2018 <- read.csv("https://raw.githubusercontent.com/nmac2000/Regen/main/Data/ClimateBC/2016/2020.csv")
#write.csv(clim2016.2021, "C:/Users/nmac2000/Documents/regen project/Regen/Data/ClimateBC/2016/2021.csv")
clim2016.2019 <- read.csv("https://raw.githubusercontent.com/nmac2000/Regen/main/Data/ClimateBC/2016/2021.csv")
...
clim2016.2017 <-select(clim2016.2017, -X) 
clim2016.2018 <-select(clim2016.2018, -X) 
clim2016.2019 <-select(clim2016.2017, -X) 
...
clim.all <- clim.all %>% 
  select(ID1, ID2, Year, MAT, MWMT, MAP, NFFD, PAS, AHM, SHM, Tave_sp, Tave_sm, Tmax_sp, Tmax_sm, Tmin_sp,
         Tmin_sm, NFFD_sp, PAS_sp, PAS_wt, PPT_sm)
```
need to decide what to do about plots with multiple plot locations
```{r}
no_duplicates <- clim.all %>% 
  group_by(ID1) %>% 
  summarise(count = n()) %>% 
  filter(count <= 6)

clim.all <- clim.all %>% 
  filter(ID1 %in% no_duplicates$ID1)
```
...
data_frames <- list(MAT, MWMT, MAP, PAS, AHM, SHM, Tave_sp, Tave_sm, Tmax_sp, Tmax_sm, Tmin_sp,
         Tmin_sm, NFFD_sp, PAS_sp, PAS_wt, PPT_sm)
zscores <- Reduce(function(x, y) merge(x, y, by = "ID1"), data_frames)

zscore.sums <- zscores %>%
  group_by(ID1) %>% 
  filter(!ID1 %in% c(1111, 13, 1460, 1461, 1467, 193)) %>% 
  summarize(hot.dry.sum = sum(across(starts_with("hot.dry"))),
            cool.wet.sum = sum(across(starts_with("cool.wet"))))

zscore.sums <- rename(zscore.sums, SampleSite_ID = "ID1")
...
climate.PLIa <- glm(PLI.f ~ hot.dry.sum ,
    family=binomial(link = "logit"), data=zscores.regen)
summary(climate.PLIa)

climate.PLIb <- glm(PLI.f ~ cool.wet.sum ,
    family=binomial(link = "logit"), data=zscores.regen)
summary(climate.PLIb)

climate.PLIc <- glm(PLI.f ~ hot.dry.sum + cool.wet.sum,
    family=binomial(link = "logit"), data=zscores.regen)
summary(climate.PLIc)

climate.FDIa <- glm(FDI.f ~ hot.dry.sum ,
    family=binomial(link = "logit"), data=zscores.regen)
summary(climate.FDIa)

climate.FDIb <- glm(FDI.f ~ cool.wet.sum ,
    family=binomial(link = "logit"), data=zscores.regen)
summary(climate.FDIb)

climate.FDIc <- glm(FDI.f ~ hot.dry.sum + cool.wet.sum,
    family=binomial(link = "logit"), data=zscores.regen)
summary(climate.FDIc)
```
...
#need to rethink these with 30 year normal in there as well.
```{r}

#2009
clim2009 <- clim2009 %>% 
  select(ID1, MAT, MWMT, MAP, NFFD, PAS, Eref, CMD, CMI, Tave_sp, Tave_sm, Tmax_sp, Tmax_sm, Tmin_sp,
         Tmin_sm, NFFD_sp, PAS_sp, Year) %>% 
  filter(Year > 0)
