...
```
```{r}
climate.PLI.3 <- glm(PLI.f ~  NFFD_sp,
                        family = binomial(link="logit"), data=climate)
summary(climate.PLI.3)

climate.PLI.3a <- glm(PLI.f ~ years_since  + NFFD_sp,
                        family = binomial(link="logit"), data=climate)
summary(climate.PLI.3a)

pairs(~ PAS + MAP + PPT_sm + NFFD_sp, data=climate)
pairs(~ MCMT + MWMT + Tmax_sm +  NFFD_sp, data=climate)
pairs(~ AHM + SHM + CMI_sm + NFFD_sp, data=climate)
```
