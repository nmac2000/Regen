...
```{r}
regen_GIS$Beers_aspect <- sin(regen_GIS$Aspect + 45) + 1
test <- data.frame(replicate(10,sample(0:360,1000,rep=TRUE)))
df <- data.frame(X1=1:360)
df$sin <- sin(df$X1 +45) +1
test$sin <- sin(test$X1 + 45) + 1
ggplot(test, aes(x=X1, y=sin)) +
  geom_point()
ggplot(df, aes(x=X1, y=sin)) +
  geom_point()

ggplot(regen_GIS, aes(x=Aspect, y=Beers_aspect)) +
  geom_point()
